...
#include "systick.h"
/* Private define ------------------------------------------------------------*/
#define LCD_REG              (*((volatile unsigned short *) 0x6C000000)) /* RS = 0 from scheme of Redbull*/
#define LCD_RAM              (*((volatile unsigned short *) 0x6C000002)) /* RS = 1 from scheme of Redbull*/
/*******************************************************************************
...
void LCD_Initialization(void)
{
	...
	#ifdef USE_16BIT_565
	LCD_WriteData(0x03);		/* 16-bit(565 format) data for Redbull*/
	#else
...
{
	uint16_t angle, Xpos_temp, Ypos_temp;

	if( Xpos > DISP_HOR_RESOLUTION || Ypos > DISP_VER_RESOLUTION )
	{
		return;
	}
	for( angle=1; angle<=radius*35; angle++ )
	{
		Xpos_temp=radius*cos(angle)+Xpos;
		Ypos_temp=radius*sin(angle)+Ypos;
		LCD_SetPoint(Xpos_temp,Ypos_temp,color);
	}
}
/******************************************************************************
...
/******************************************************************************
* Function Name  : LCD_FillArea
* Description    : added by SG
* Input          : - start_x: start column
*                  - start_y: start row
*		   - end_x: end column
*		   - end_y: end row
...
/******************************************************************************
* Function Name  : LCD_EnterInvertMode
* Description    : added by SG
* Input          : None
* Output         : None
* Return         : None
* Attention		 : None
*******************************************************************************/
__inline void LCD_EnterInvertMode(void)
{
	LCD_WriteCommand(CMD_ENT_INVERT_MODE);
}
/******************************************************************************
* Function Name  : LCD_ExitInvertMode
* Description    : added by SG
* Input          : None
* Output         : None
* Return         : None
* Attention		 : None
*******************************************************************************/
__inline void LCD_ExitInvertMode(void)
{
	LCD_WriteCommand(CMD_EXIT_INVERT_MODE);
}
/******************************************************************************
* Function Name  : LCD_FlipVertical(void)
* Description    : added by SG
* Input          : None
* Output         : None
* Return         : None
* Attention		 : None
*******************************************************************************/
__inline void LCD_FlipVertical(void)
{
	LCD_WriteCommand(CMD_SET_ADDR_MODE);
	LCD_WriteData(0b10000001);
}
/******************************************************************************
* Function Name  : LCD_FlipHorizontal(void)
* Description    : added by SG
* Input          : None
* Output         : None
* Return         : None
* Attention		 : None
*******************************************************************************/
__inline void LCD_FlipHorizontal(void)
{
	LCD_WriteCommand(CMD_SET_ADDR_MODE);
	LCD_WriteData(0b10000010);
}
/******************************************************************************
* Function Name  : PutChar_x2
* Description    : added by SG
* Input          : - Xpos: 
*                  - Ypos: 
*				   - ASCI:
...
* Attention		 : None
*******************************************************************************/
void PutChar_x2( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
    uint16_t i, j;
    uint8_t buffer[16], tmp_char;
    ...
    for( i=0; i<16; i++ )
    {
        ...
        for( j=0; j<8; j++ )
        {
            if(((tmp_char >> (7 - j)) & 0x01) == 0x01)
            {
                LCD_SetPoint( Xpos +2*j, Ypos + 2*i, charColor );
                LCD_SetPoint( Xpos + 2*j+1, Ypos + 2*i, charColor );
                LCD_SetPoint( Xpos +2*j, Ypos + 2*i+1, charColor);
                LCD_SetPoint( Xpos + 2*j+1, Ypos + 2*i+1, charColor );
            }
            else
            {
                LCD_SetPoint( Xpos +2*j, Ypos + 2*i, bkColor );
                LCD_SetPoint( Xpos +2*j+1, Ypos +2*i, bkColor );
                LCD_SetPoint( Xpos +2*j, Ypos + 2*i+1, bkColor );
                LCD_SetPoint( Xpos +2*j+1, Ypos +2*i+1, bkColor );
            }
        }
    }
}
...
void GUI_Text_x2(uint16_t Xpos, uint16_t Ypos, char *str, uint16_t Color, uint16_t bkColor)
{
    ...
    {
        TempChar = *str++;  
        PutChar_x2( Xpos, Ypos, TempChar, Color, bkColor );    
